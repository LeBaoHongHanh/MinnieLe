...
def auto_fetch_email(interval, email_state):
    while True:
        ...
        # Lọc thư
        email_content = []
        for i in range(1, len(email_list) + 1):
            email_content[i] = parse_email_content(email_list[i])
            print(email_content[i])
...
def parse_email_content(email_content):
    ...
    for line in lines:
        ...
        # Bắt đầu đọc nội dung sau dòng trống
        elif line == "":
